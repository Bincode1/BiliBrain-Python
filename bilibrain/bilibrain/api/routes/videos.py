...
async def get_video_transcript(
    bvid: str, runtime: Runtime = Depends(get_runtime)
) -> dict[str, Any]:
    return await build_transcript_payload(runtime, bvid)
...
async def get_process_status(
    ...
    return await build_status_payload(runtime, bvid)
...
async def reset_video_processing_route(
    bvid: str, runtime: Runtime = Depends(get_runtime)
) -> dict[str, Any]:
...
async def reset_all_video_processing_route(
...
async def update_video_tags(
    ...
    await require_video(runtime, bvid)
    tags = await runtime.db.set_video_tags(bvid, payload.tags)

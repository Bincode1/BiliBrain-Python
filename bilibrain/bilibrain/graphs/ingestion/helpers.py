...
async def require_video(runtime: Runtime, bvid: str) -> dict[str, Any]:
...
def hydrate_audio_step_state(
    runtime: Runtime, video: dict[str, Any] | None, state: dict[str, dict[str, Any]]
) -> None:
        ...
        state["audio"].update(
            {
                ...
                "url": runtime.audio_storage.get_audio_url(object_key),
            }
        )


async def prepare_audio_input_file(
    runtime: Runtime, video: dict[str, Any], target_path: Path
) -> Path:
    provider = video.get("audio_storage_provider")
    object_key = video.get("audio_object_key")
    if provider and object_key:
        return await runtime.audio_storage.download_audio(
            str(provider), str(object_key), target_path
        )

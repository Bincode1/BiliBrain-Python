...
async def load_conversation_context(state: QAState) -> dict[str, Any]:
    ...
    context = build_conversation_context(runtime, conversation_id=conversation_id)
...
async def append_user_message(state: QAState) -> dict[str, Any]:
    ...
    refresh_context_stats_after_message(runtime, conversation_id=conversation_id, message=user_message)
...
async def plan_query_route(state: QAState) -> dict[str, Any]:
    ...
    if not policy["planner_enabled"] or not should_use_planner(query):
        ...
        route = "history_only" if intent["intent"] == "history" else ("summary_only" if intent["intent"] != "detail_qa" else "chunk_only")
        use_history = True
        retrieval_mode = "none" if route == "history_only" else ("summary" if route == "summary_only" else "chunk")
        query_plan = None

        status_text = status_map.get(route, "正在检索回答...")

        if writer:
            writer({"_status": status_text, "_route_mode": route, "_mode": retrieval_mode})
    else:
        if writer:
            writer({"_status": "正在规划回答策略..."})

        runtime.qwen.ensure_configured()
        scope_description = describe_query_scope(runtime, folder_id=folder_id, bvid=bvid, scope_mode=scope_mode)
        ...
        if writer:
            writer({"_route_mode": route, "_mode": retrieval_mode})
...
async def prepare_data_for_answer(state: QAState) -> dict[str, Any]:
    ...
    matches = rerank_search_hits(query=query, hits=filtered_hits, limit=policy["rerank_top_k"])
...
async def append_assistant_message(state: QAState) -> dict[str, Any]:
    ...
    route_mode = state.get("route_mode") or "chunk_only"
    policy = _execution_policy(state)

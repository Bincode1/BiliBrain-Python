...
def _make_runtime(**kwargs):
    ...
    def track_cache_task(key, task):
        runtime.cache_tasks[key] = task

        def cleanup(done_task):
            runtime.cache_tasks.pop(key, None)
            try:
                done_task.result()
    ...
    runtime.track_cache_task = track_cache_task
    return runtime


def test_merge_transcript_segments_breaks_on_large_gap():
    segments = [
...
def test_merge_transcript_segments_uses_sentence_aware_overlap():
    segments = [
...
def test_merge_transcript_segments_prefers_token_budget_over_small_char_target():
    segments = [
...
def test_merge_transcript_segments_can_merge_across_coarse_asr_ranges():
    segments = [
        {
            "from": 0,
            "to": 90,
...
def test_merge_transcript_segments_respects_max_token_limit():
    segments = [
        {
            "from": 0,
            "to": 6,
...
def test_classify_query_intent_defaults_to_detail_qa():
    ...
    assert result == {"intent": "detail_qa", "scope": "video"}
...
class _FakeBili:
    def __init__(self, *, folders=None, videos=None, search_result=None):
        self.folders = folders or []
        self.videos = videos or []
        self.search_result = search_result or {"keyword": "", "page": 1, "page_size": 12, "total": 0, "results": []}
        self.list_folders_calls = 0
        self.list_folder_videos_calls = 0
        self.search_videos_calls = []
    ...
    async def search_videos(self, keyword, *, page=1, page_size=12, order="totalrank"):
        self.search_videos_calls.append(
            {
                "keyword": keyword,
                "page": page,
                "page_size": page_size,
...
def test_search_bilibili_videos_for_folder_defaults_to_folder_title():
    folder_id = 10
    db = _FakeDb(
        folder_rows={folder_id: {"folder_id": folder_id, "title": "AI Agent", "media_count": 5}},
    )
    bili = _FakeBili(
        search_result={
            "keyword": "AI Agent",
            ...
            "page_size": 12,
            "total": 1,
            "results": [{"bvid": "BV1", "title": "Agent 实战"}],
        }
    )
    runtime = SimpleNamespace(db=db, bili=bili)

    payload = asyncio.run(search_bilibili_videos_for_folder(runtime, folder_id))

    assert payload["folder"]["folder_id"] == folder_id
    assert payload["keyword"] == "AI Agent"
    assert payload["results"][0]["bvid"] == "BV1"
    assert bili.search_videos_calls == [
        {"keyword": "AI Agent", "page": 1, "page_size": 12, "order": "totalrank"}
    ]
